...
1. Move task to in-progress: `mpga-plugin/bin/mpga.sh board claim <task-id>`
...
6. If reviewer passes → move to done: `mpga-plugin/bin/mpga.sh board move <id> done`
7. Record evidence produced: `mpga-plugin/bin/mpga.sh board update <id> --evidence-add "<link>"`
...
- If context budget is low during execution: `mpga-plugin/bin/mpga.sh session budget`
