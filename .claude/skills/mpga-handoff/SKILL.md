...
1. Check context budget:
   ```
   node ./.mpga-runtime/cli/dist/index.js session budget
   ```
...
3. Capture task state from the board:
   ```
   node ./.mpga-runtime/cli/dist/index.js board show
   ```
...
5. Save handoff document:
   ```
   node ./.mpga-runtime/cli/dist/index.js session handoff --accomplished "<summary>"
   ```

6. Log the session:
   ```
   node ./.mpga-runtime/cli/dist/index.js session log "<brief description of work done>"
   ```
