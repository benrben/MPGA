...
flowchart TD
    ...
    B --> C[Read existing scope document — what do we KNOW already?]
    C --> D[Navigate to files in assigned scope — BOOTS on the ground]
    D --> E[Prioritize changed or high-traffic files — the IMPORTANT ones first]
    E --> F[For each file: read code, trace call chains — TOTAL understanding]
    F --> G[Fill Summary section: MPGA voice — what makes this module GREAT]
