...
const EVIDENCE_HEALTH_WARN_PCT = 50;

async function handleEvidenceVerify(opts: { scope?: string; json?: boolean }): Promise<void> {
  ...
  for (const scope of report.scopes) {
    const icon =
      scope.healthPct >= EVIDENCE_HEALTH_GOOD_PCT
        ? chalk.green('✓')
        : scope.healthPct >= EVIDENCE_HEALTH_WARN_PCT
          ? chalk.yellow('⚠')
          : chalk.red('✗');
    console.log(
  ...
  } else if (report.overallHealthPct >= EVIDENCE_HEALTH_GOOD_PCT) {
...
async function handleEvidenceHeal(opts: { auto?: boolean; scope?: string }): Promise<void> {
  const projectRoot = findProjectRoot() ?? process.cwd();
  const config = loadConfig(projectRoot);
...
async function handleEvidenceCoverage(opts: { min: string }): Promise<void> {
  const projectRoot = findProjectRoot() ?? process.cwd();
  const config = loadConfig(projectRoot);
...
function handleEvidenceAdd(scopeName: string, link: string): void {
  const projectRoot = findProjectRoot() ?? process.cwd();
