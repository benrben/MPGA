...
export function registerScope(program: Command): void {
  ...
  cmd
    .command('list')
    .description('Show all scopes with health status')
    .action(() => {
  ...
  cmd
    .command('add <name>')
    .description('Create a new empty scope document')
    .action((name: string) => {
      const projectRoot = findProjectRoot() ?? process.cwd();
      const scopesDir = getScopesDir(projectRoot);
...
`;
  ...
  cmd
    .command('remove <name>')
    .description('Archive a scope document')
    .action((name: string) => {
      const projectRoot = findProjectRoot() ?? process.cwd();
      const scopePath = path.join(getScopesDir(projectRoot), `${name}.md`);
    ...
    .action((question: string) => {
      ...
      for (const file of files) {
        ...
        if (score > 0) {
          ...
          const excerpt = content.slice(start, start + 200).replace(/\n/g, ' ');
