...
3. Do tests start with degenerate cases? (empty input, null, zero should be the first tests)
...
6. Testability: are external dependencies (DB, APIs, filesystem) accessed through interfaces/abstractions that can be substituted in tests? Flag direct coupling to external services.
