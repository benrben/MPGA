...
1. Check context budget:
   ```
   ${CLAUDE_PLUGIN_ROOT}/bin/mpga.sh session budget
   ```
...
3. Save handoff document:
   ```
   ${CLAUDE_PLUGIN_ROOT}/bin/mpga.sh session handoff --accomplished "<summary>"
   ```

4. Log the session:
   ```
   ${CLAUDE_PLUGIN_ROOT}/bin/mpga.sh session log "<brief description of work done>"
   ```
