...
describe('export --claude', () => {
  ...
  it('creates Codex skills with discoverable skill names in frontmatter', async () => {
    const program = createProgram();
    registerExport(program);
    await program.parseAsync(['export', '--codex'], { from: 'user' });

    const skillPath = path.join(tmpDir, '.codex', 'skills', 'mpga-plan', 'SKILL.md');
    expect(fs.existsSync(skillPath)).toBe(true);

    const content = fs.readFileSync(skillPath, 'utf-8');
    expect(content).toContain('name: mpga-plan');
...
describe('AGENTS metadata validity', () => {
  it('every agent has required fields', async () => {
    ...
    for (const field of requiredFields) {
      // Each field should appear at least once per agent (10 agents)
  ...
  it('exports the full skill catalog used by project integrations', async () => {
    const exportSource = fs.readFileSync(path.join(__dirname, 'export', 'agents.ts'), 'utf-8');
    ...
    for (const skill of expectedSkills) {
      expect(exportSource).toContain(`'${skill}'`);
    }
  });
});
