...
describe('board show', () => {
  ...
  it('writes live snapshot and html artifacts with board live', async () => {
    const tasksDir = path.join(tmpDir, 'MPGA', 'board', 'tasks');
    fs.writeFileSync(
      path.join(tasksDir, 'T001-live.md'),
      makeTaskFile('T001', 'Live board task', 'todo'),
    );

    const program = makeProgram();
    registerBoard(program);

    await program.parseAsync(['node', 'mpga', 'board', 'live']);

    const liveDir = path.join(tmpDir, 'MPGA', 'board', 'live');
    expect(fs.existsSync(path.join(liveDir, 'snapshot.json'))).toBe(true);
    expect(fs.existsSync(path.join(liveDir, 'index.html'))).toBe(true);
  });
});
