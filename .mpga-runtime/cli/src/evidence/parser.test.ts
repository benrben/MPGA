...
describe('parseEvidenceLink', () => {
  ...
  it('parses symbol-based format with line hint', () => {
    const result = parseEvidenceLink('[E] src/board/board.ts#checkWipLimit:170');
    expect(result).toMatchObject({
      type: 'valid',
  ...
  it('parses symbol-based format without line hint', () => {
    const result = parseEvidenceLink('[E] src/board/board.ts#checkWipLimit');
    expect(result).toMatchObject({
      type: 'valid',
      filepath: 'src/board/board.ts',
      symbol: 'checkWipLimit',
  ...
  it('parses symbol-based format with description after dash separator', () => {
    const result = parseEvidenceLink(
  ...
  it('parses symbol-based stale format', () => {
    const result = parseEvidenceLink('[Stale:2026-03-20] src/foo.ts#myFunc:42');
    expect(result).toMatchObject({
      type: 'stale',
      staleDate: '2026-03-20',
      filepath: 'src/foo.ts',
  ...
  it('parses symbol-based deprecated format', () => {
    const result = parseEvidenceLink('[Deprecated] src/old.ts#legacyInit:5');
    expect(result).toMatchObject({
      type: 'deprecated',
      filepath: 'src/old.ts',
      symbol: 'legacyInit',
  ...
  it('strips backticks from symbol-based format', () => {
    const result = parseEvidenceLink('[E] `src/foo.ts`#`bar`:10');
    expect(result?.filepath).toBe('src/foo.ts');
    expect(result?.symbol).toBe('bar');
...
`;
  ...
  it('extracts mixed old-format and symbol-based links', () => {
    const content = `
## Evidence
[E] src/a.ts:1-10 :: funcA()
[E] src/b.ts#funcB:20
...
`;
    ...
    expect(links[0].symbol).toBe('funcA');
    expect(links[0].startLine).toBe(1);
    expect(links[0].endLine).toBe(10);
    expect(links[1].symbol).toBe('funcB');
    expect(links[1].startLine).toBe(20);
    expect(links[1].endLine).toBeUndefined();
    expect(links[2].type).toBe('unknown');
    expect(links[3].symbol).toBe('funcC');
    expect(links[3].description).toBe('description here');
  });
});
...
describe('formatEvidenceLink', () => {
  ...
  it('formats a valid link without line range (symbol-based)', () => {
    const link: EvidenceLink = {
      raw: '',
      type: 'valid',
      filepath: 'src/a.ts',
    ...
    // Symbol without endLine uses new symbol-based format
  ...
  it('formats symbol-based valid link with line hint', () => {
    const link: EvidenceLink = {
      raw: '',
      type: 'valid',
      filepath: 'src/board/board.ts',
    ...
    expect(formatEvidenceLink(link)).toBe('[E] src/board/board.ts#checkWipLimit:170');
  });

  it('formats symbol-based valid link without line hint', () => {
    const link: EvidenceLink = {
      raw: '',
      type: 'valid',
      filepath: 'src/board/board.ts',
    ...
    expect(formatEvidenceLink(link)).toBe('[E] src/board/board.ts#checkWipLimit');
  });

  it('formats symbol-based valid link with description', () => {
    const link: EvidenceLink = {
      raw: '',
      type: 'valid',
      filepath: 'src/board/board.ts',
    ...
    expect(formatEvidenceLink(link)).toBe(
      '[E] src/board/board.ts#checkWipLimit:170 — WIP limit validation',
    );
  });

  it('formats symbol-based stale link', () => {
    const link: EvidenceLink = {
      raw: '',
      type: 'stale',
      staleDate: '2026-03-20',
    ...
    expect(formatEvidenceLink(link)).toBe('[Stale:2026-03-20] src/foo.ts#myFunc:42');
  });
});
