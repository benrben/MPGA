  ...
  if (link.symbol) {
    ...
    if (location) {
      if (isSymbolBasedLink(link)) {
        ...
        const hintDrifted = link.startLine != null && link.startLine !== location.startLine;
        return {
          status: hintDrifted ? 'healed' : 'valid',
          confidence: CONFIDENCE_AST_ANCHOR,
          startLine: location.startLine,
          endLine: location.endLine,
          healedFrom: hintDrifted
            ? `line hint drifted: was ${link.startLine}, now ${location.startLine}`
            : undefined,
        };
      } else {
